...
if __name__ == "__main__":
    ...
    thresh_param = 4.0
    iter_param = 250
    data_path_list = [os.path.join("data","airplane-"+str(i)) for i in range(4,21)]
    for data in data_path_list:
        ...
        imgs = vl.get_imgs()
        truth = vl.get_truth()
        vs = video.videoSplitter(rot=20, trans=20, crop=int(width*0.6), bright=(10,25), height=height, width=width)
        ...
        for i, img in enumerate(imgs):
            ...
            result = pano.combine(left, right, h, blend=True)
            ...
            if i == 0:
                ...
                cv2.waitKey(1)
        imageio.mimsave("airplane4_boxes_bright.gif", gif_imgs)
        break
